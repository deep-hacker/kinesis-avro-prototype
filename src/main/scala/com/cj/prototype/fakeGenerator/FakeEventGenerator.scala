...
class FakeEventGenerator(productChangeHandler: ProductChangeHandler) {
  ...
  val product:CJProduct = CJProduct(12345,11345,"Demo",534,"dummmy product","Demo12345","Dummy",System.currentTimeMillis(), AmountWithCurrency(54.00,"USD"),true,System.currentTimeMillis(),false)
  var productList = ArrayBuffer[String]()
  def createProduct() = {
    populateProductList()
    for(_ <- 1 to 500){
      val product = CJProduct(12345,11345,"CJ Demo",534,s"dummy product${random.nextInt(500)}",s"Dummy ${random.nextInt(5000)}",productList(random.nextInt(productList.size)),System.currentTimeMillis(),AmountWithCurrency(random.nextDouble*600.00,"USD"),true,System.currentTimeMillis(),false)
